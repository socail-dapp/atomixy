                            ...
                            <Popover.Panel className="absolute z-10 w-screen max-w-sm mt-3 transform  right-0  ">
                                <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 ">
                                    <div className='flex row'>
                                        <div
                                            onClick={() => setTab(1)}
                                            className={`w-full flex justify-center align-middle p-3 bg-orange-400 text-white font-medium cursor-pointer ${!tabs && `opacity-25`}`}>Request</div>
                                        <div
                                            onClick={() => setTab(0)}
                                            className={`w-full flex justify-center align-middle p-3 bg-orange-400 text-white font-medium cursor-pointer ${tabs && `opacity-25`}`}>History</div>
                                    </div>
                                    ...
                                    {<div className="relative  max-h-80 bg-white p-4 overflow-scroll ">
                                        {dataArray?.map((item,) => (
                                            ...
                                            >
                                                ...
                                                <div
                                                    className="flex items-center justify-center flex-shrink-0 text-white h-12 w-12 ">
                                                    {/* <item.icon aria-hidden="true" /> */}
                                                    {item?.uuid === data?.currentVersion?.uuid ? <IconThree /> :
                                                        <div
                                                            onClick={() => {
                                                                onOpen()
                                                                setItemModal(item)
                                                ...
                                                <div
                                                    ...
                                                    <p className={"text-sm font-medium " + `${item?.uuid !== selectedVersion?.uuid ? `text-gray-900` : `text-orange-700`}`}>
                                                        {item?.versionName}
                                                    </p>
                                                    <div className='flex row justify-between align-bottom'>
                                                        <p className={"text-sm " + `${item?.uuid !== selectedVersion?.uuid ? `text-gray-500` : `text-orange-700`}`}>
                                                            By {item.createdBy}
                                                            {/* date */}
                                                        </p>
                                    ...
                                    <div className="p-4 bg-gray-50">
                                        ...
                                        >
                                            ...
                                            <span className="block text-sm text-gray-500">
                                                {
                                                    tabs ? `List of requested version, must be approved to change the flow into main` : `List of history approved version, can re-approved previous version`
                                                }
                                            </span>
                                        </a>
                                    </div>
