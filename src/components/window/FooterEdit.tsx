...
export default function FooterEdit({ onRemove, onAdd }) {
  ...
  return (
    <div className="fixed z-30 rounded-md p-4 border backdrop-blur-sm w-full md:w-1/2 right-0 bottom-3">
      ...
      />
      {isEditWindow && (
        <Button onClick={onOpen} className={`bg-rose-400 hover:bg-red-600 `}>
          Delete
        </Button>
      )}
      <Button onClick={onAdd} className={`float-right px-6`}>
        {isEditWindow ? `Update` : `Add Node`}
      </Button>
      <Button
        onClick={() => {
          alert("This feature will update the flow too, but under development");
        }}
        className={`float-right px-6 mr-3`}
      >
        {isEditWindow ? `Update & Save Version` : `Add Node & Save Version`}
      </Button>
    </div>
  );
}
