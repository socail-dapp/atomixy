...
export default function Settings({
  ...
  return (
    <PanelWrapper>
      ...
      <div
        className="grid place-items-center p-4 py-10 border mb-4 "
      ...
      >
        <div
          ...
          className="rounded-md text-white p-3 px-5 my-10"
        >
          {detail?.titleCapsule}
        </div>
      </div>
      <div className="flex row w-full ">
        <div className="col-span-6 w-full">
          <Input
            ...
            onChange={(titleCapsule: string) =>
              _updateDetail("titleCapsule", titleCapsule)
            }
          />
        </div>
      ...
      <div className="flex row text-sm font-medium text-gray-700 overflow-scroll">
        <div className="pr-6 w-full">
          ...
          <HexColorPicker
            color={detail?.colorBg}
            onChange={(v) => _updateDetail(`colorBg`, v)}
          />
        </div>
...
const LinkConnect = ({ item, isFor, onRemoveLink }) => {
  return (
    <div
      style={{
        background: isFor
          ? item?.to?.detail?.colorBg
          : item?.from?.detail?.colorBg,
        color: isFor
          ? item?.to?.detail?.colorText
          : item?.from?.detail?.colorText,
      }}
    ...
    >
      ...
      &nbsp;
      <Button onClick={() => onRemoveLink(item?.id)} className={`bg-gray-600`}>
        unlink
      </Button>
    </div>
  );
