            ...
            <br /> */}
                <Input
                    ...
                    onChange={(title: string) => {
                        _updateDetail('title', title, 'titleCapsule', title?.substring(0, 20));
                    }}
                />
                <br />
...
export const DetailDisplay = ({ setEdit, detail }) => {
    ...
    return (
        <div>
            <div className=' text-2xl text-gray-700 font-semibold'>{detail?.title}</div>
            <div className='flex row mx-0'>
                {state?.tags?.map((item, i) => <div key={i} className='bg-slate-400 my-3 mr-2 px-3 text-white text-xs'>{item}</div>)}
            </div>
            <div className='text-base text-gray-600 font-normal'>{detail?.description}</div>
            ...
            <div className='flex row mx-0'>
                {options?.map((item, i) => <div
                    ...
                    key={i} className='my-3 mr-2 pr-3 text-blue-600 text-sm cursor-pointer'>{item}</div>)}
            </div>
